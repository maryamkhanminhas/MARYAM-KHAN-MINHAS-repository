...
// Generate a report of all customers who stayed in the hotel
void generateReport() {
    cout << "\nCustomer Report:\n";
    for (int i = 0; i < MAX_ROOMS; ++i) {
        if (rooms[i].status == "Occupied") {
...
// Update room status (e.g., for cleaning or availability)
void updateRoomStatus(int roomNo) {
    ...
    cout << "Enter new status for Room " << roomNo << " (Available/Occupied): ";
    string status;
    cin >> status;
